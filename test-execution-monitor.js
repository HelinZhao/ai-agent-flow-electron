...
async function testWorkflowExecutionMonitoring() {
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      try {
        const progressResponse = await axios.get(`${API_BASE_URL}/execute-workflow/progress/${executionId}`);
        ...
        console.log(`   已完成节点: ${progress.metrics.completedNodes}/${progress.metrics.totalNodes}`);
    ...
    if (progress) {
      ...
      console.log(`   总耗时: ${progress.metrics.duration ? `${progress.metrics.duration}ms` : '未知'}`);
      ...
      // 显示执行日志
      console.log('\n📝 执行日志:');
      progress.logs.slice(-10).forEach((log, index) => { // 只显示最后10条日志
