...
export default function SettingsKnowledge(): React.JSX.Element {
  ...
  return (
    <div className='pb-4'>
      ...
      {/* Embedding 模型状态卡片 */}
      {modelExists !== null && (
        <div className={`mb-5 p-4 rounded-xl border transition-colors ${modelExists
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-3 min-w-0">
              <div className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center ${modelExists
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <CustomInput type="number" min={CHUNK_SIZE_RANGE.min} max={CHUNK_SIZE_RANGE.max} {...register('chunkSize', { valueAsNumber: true })} size='sm' />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">分块重叠</label>
                    <CustomInput type="number" min={CHUNK_OVERLAP_RANGE.min} max={CHUNK_OVERLAP_RANGE.max} {...register('chunkOverlap', { valueAsNumber: true })} size='sm' />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">检索数量</label>
                    <CustomInput type="number" min={TOP_K_RANGE.min} max={TOP_K_RANGE.max} {...register('topK', { valueAsNumber: true })} size='sm' />
                  </div>
                </div>
              </div>
            )}
            ...
            {kbType === 'external' && (
              <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700 space-y-3">
                ...
                <div>
                  ...
                  <CustomInput type="password" {...register('apiKey')} placeholder="Bearer token" size='sm' />
                </div>
